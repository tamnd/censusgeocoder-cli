...
describes what users get. Pick a format with `-o`, or let geocoder choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
geocoder <command> -o table   # aligned columns for reading
geocoder <command> -o jsonl   # one JSON object per line, for piping
geocoder <command> -o json    # a single JSON array
geocoder <command> -o csv     # spreadsheet friendly
geocoder <command> -o tsv     # tab-separated
geocoder <command> -o url     # just the URL column
geocoder <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
geocoder <command> --fields id,title,url
```
...
```bash
geocoder <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
geocoder <command>            # a table, because this is a terminal
geocoder <command> | wc -l    # JSONL, because this is a pipe
```
